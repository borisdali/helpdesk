...
func (s *Store) Record(ctx context.Context, event *Event) error {
	...
	if event.Outcome != nil {
		...
		// Surface target-scope drift, protocol violations, fabrication mismatch, and
		// unverified evidence as distinct outcome strings so QueryJourneys can elevate
		// the journey outcome — checked in this order since a single event's
		// TargetDrift, ProtocolViolation, Mismatch, and UnverifiedEvidence are
		// independent signals (see checkTargetScope/protocolViolation/
		// checkEvidenceProvenance in cmd/gateway/playbooks.go), but the stored
		// outcome_status column holds only one value per event. A trace where
		// multiple signals fire (on the same or different hops) is still fully
		// discoverable either way, via the independent HasMismatch/HasTargetDrift/
		// HasProtocolViolation/HasUnverifiedEvidence booleans on the journey summary
		// rather than this single string. Clean verifications are recorded as
		// "verified".
		switch {
		...
		case len(event.DelegationVerification.UnverifiedEvidence) > 0:
			outcomeStatus = "unverified_evidence"
		default:
...
type JourneySummary struct {
	...
	// HasUnverifiedEvidence is true when any delegation_verification event for this
	// journey has a non-empty UnverifiedEvidence — meaning a hop's EVIDENCE quote
	// couldn't be matched to any real tool_execution output recorded for that hop
	// (content-provenance, fabrication-detection Layer 3, v0.28.0). Computed
	// independently of which outcome string (outcomePriority) wins for the trace,
	// mirroring HasMismatch/HasTargetDrift/HasProtocolViolation, so a trace with
	// multiple signals on different hops is still discoverable via any boolean
	// regardless of the 4-way tie at priority 9.
...
func (s *Store) QueryJourneys(ctx context.Context, opts JourneyOptions) ([]JourneySummary, error) {
	...
	type traceData struct {
		...
		hasUnverifiedEvidence bool // true when any delegation_verification event has unverified_evidence outcome
	}
	...
	for rows2.Next() {
		...
		if eventType == string(EventTypeVerificationOutcome) || eventType == string(EventTypeDelegationVerification) {
			...
			// Track unverified evidence: "unverified_evidence" is the outcome stored
			// for delegation_verification events where UnverifiedEvidence is
			// non-empty — independent of hasMismatch/hasTargetDrift/
			// hasProtocolViolation above (see the 4-way tie at outcomePriority 9),
			// so checked separately rather than as an else-if.
			if eventType == string(EventTypeDelegationVerification) &&
				outcomeStatus.Valid && outcomeStatus.String == "unverified_evidence" {
				d.hasUnverifiedEvidence = true
			}
			continue
		}
	...
	for _, id := range traceIDs {
		...
		summaries = append(summaries, JourneySummary{
			...
			HasUnverifiedEvidence: d.hasUnverifiedEvidence,
		})
	}
...
//
//	unverified_claim(9) = target_drift_detected(9) = protocol_violation(9) = unverified_evidence(9) > error(8) > denied(7) > escalation_required(6) > verified_failed(5)
//	> verified_warning(4) > approved(3) > verified_ok(2) > success(1) > verified(0.5) > unknown(0)
//
// unverified_claim, target_drift_detected, protocol_violation, and unverified_evidence
// are deliberately tied, not ranked — all four represent "this agent's output can't be
// trusted as-is" for different reasons (no evidence at all, evidence pointing at the
// wrong target, the agent skipping the required response protocol entirely, or a cited
// EVIDENCE quote that never appeared in any real tool output), not different
// severities. The tie is safe because HasMismatch/HasTargetDrift/HasProtocolViolation/
// HasUnverifiedEvidence on JourneySummary are computed independently of which outcome
// string wins this priority comparison, so a trace where multiple fire is still fully
// discoverable via any of the booleans even though only one outcome string is
// displayed.
