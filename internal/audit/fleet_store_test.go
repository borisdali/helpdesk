...
func TestFleetStore_PlanTraceID(t *testing.T) {
	fs := newFleetTestStore(t)
	ctx := context.Background()

	// Job generated by the NL planner carries a plan_trace_id.
	planned := &FleetJob{
		Name:        "planned-job",
		SubmittedBy: "fleet-runner",
		JobDef:      `{"name":"planned-job"}`,
		PlanTraceID: "plan_abc12345",
	}
	// Job submitted manually has no plan_trace_id.
	manual := &FleetJob{
		Name:        "manual-job",
		SubmittedBy: "fleet-runner",
		JobDef:      `{"name":"manual-job"}`,
	}

	if err := fs.CreateJob(ctx, planned); err != nil {
		t.Fatalf("CreateJob planned: %v", err)
	}
	if err := fs.CreateJob(ctx, manual); err != nil {
		t.Fatalf("CreateJob manual: %v", err)
	}

	// GetJob round-trips PlanTraceID.
	got, err := fs.GetJob(ctx, planned.JobID)
	if err != nil {
		t.Fatalf("GetJob: %v", err)
	}
	if got.PlanTraceID != "plan_abc12345" {
		t.Errorf("PlanTraceID = %q, want plan_abc12345", got.PlanTraceID)
	}

	// Manual job has empty PlanTraceID.
	gotManual, err := fs.GetJob(ctx, manual.JobID)
	if err != nil {
		t.Fatalf("GetJob manual: %v", err)
	}
	if gotManual.PlanTraceID != "" {
		t.Errorf("manual job PlanTraceID = %q, want empty", gotManual.PlanTraceID)
	}

	// ListJobs filtered by plan_trace_id returns only the planned job.
	filtered, err := fs.ListJobs(ctx, FleetJobQueryOptions{PlanTraceID: "plan_abc12345", Limit: 50})
	if err != nil {
		t.Fatalf("ListJobs(PlanTraceID): %v", err)
	}
	if len(filtered) != 1 {
		t.Fatalf("ListJobs(PlanTraceID) = %d jobs, want 1", len(filtered))
	}
	if filtered[0].JobID != planned.JobID {
		t.Errorf("filtered job = %q, want %q", filtered[0].JobID, planned.JobID)
	}
}
