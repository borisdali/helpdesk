...
3. [Informed gate](#informed-gate)
   ...
   - [Troubleshooting warning signals](#troubleshooting-warning-signals)
...
5. [API](#api)
   ...
   - [Delete a Playbook](#delete-a-playbook)
   - [Run a Playbook](#run-a-playbook)
5. [Run tracking](#run-tracking)
   - [Run lifecycle](#run-lifecycle)
   - [Get a specific run](#get-a-specific-run)
   - [List runs for a Playbook](#list-runs-for-a-playbook)
   - [Get run statistics](#get-run-statistics)
   - [Record an outcome](#record-an-outcome)
   - [Run events (chain of thought)](#run-events-chain-of-thought)
   - [Operator feedback](#operator-feedback)
   - [Diagnosis accuracy and what counts toward it](#diagnosis-accuracy-and-what-counts-toward-it)
   - [Incident narrative](#incident-narrative)
   - [`PlaybookRun` object](#playbookrun-object)
6. [Adaptive triage](#adaptive-triage)
   - [Entry points](#entry-points)
   - [Escalation graph](#escalation-graph)
   - [What "cross-domain" is actually buying you](#what-cross-domain-is-actually-buying-you)
   - [Requires-evidence warnings](#requires-evidence-warnings)
   - [Structured diagnostic report](#structured-diagnostic-report)
   - [Structured escalation signal](#structured-escalation-signal)
   - [Continuity threading](#continuity-threading)
   - [Approval modes](#approval-modes)
7. [`agent_approve` execution mode](#agent_approve-execution-mode)
   - [Reactive re-planning](#reactive-re-planning)
   - [When to use each execution mode](#when-to-use-each-execution-mode)
   - [Starting an `agent_approve` run](#starting-an-agent_approve-run)
   - [Approving a step](#approving-a-step)
   - [Full shell example](#full-shell-example)
   - [Automated approval (CI and fault tests)](#automated-approval-ci-and-fault-tests)
   - [Interactive approval (human-in-the-loop demo)](#interactive-approval-human-in-the-loop-demo)
   - [Step tracking](#step-tracking)
   - [Diagnosis / remediation playbook split](#diagnosis--remediation-playbook-split)
8. [Importing Playbooks](#importing-playbooks)
   - [YAML import format](#yaml-import-format)
9. [Life of an Incident](#life-of-an-incident)
10. [Playbook authoring guidance](#playbook-authoring-guidance)
    - [Writing effective `description` fields](#writing-effective-description-fields)
    - [Writing effective `guidance` fields](#writing-effective-guidance-fields)
    - [Escalation conditions](#escalation-conditions)
11. [Lifecycle example](#lifecycle-example)
12. [Playbook schema reference](#playbook-schema-reference)
    - [`PlaybookRunStats` object](#playbookrunstats-object)
...
flowchart TD
    ...
    Signal -- "No — neither line resolved to a real target<br/>(covers both 'line omitted entirely' and<br/>explicit 'ESCALATE_TO: none' — loop entry<br/>doesn't distinguish them; protocolViolation() below does)" --> Gap
    ...
    AlreadyGated -- No --> ForceGates{"Do any force-gates fire?<br/>low_confidence (CONFIDENCE < 50%)<br/>objective_evidence (verified tool evidence)<br/>trust_not_earned (no STABLE+CLEAN+attribution-consistent cert)"}
    ...
    Gap["recordSignalLessWarnings runs<br/>(unconditional, every signal-less hop):<br/>objective_evidence → evidence_warnings<br/>protocol_violation → warnings + protocol_violation:true<br/>+ new Journey outcome (no gate — no target to gate into).<br/>Fires only when the signal line was omitted entirely —<br/>explicit 'ESCALATE_TO: none' does NOT trip this (SawSignalLine=true).<br/>low_confidence → warnings (visibility only, not counted toward CLEAN)"]
    Gap --> GateOptIn{"gate_escalation:true AND<br/>remediation_series_id set on<br/>the ORIGINAL request?"}
    GateOptIn -- No --> ResolvedWarned[["Response returns with whatever<br/>warnings fired above.<br/>outcome: resolved/unknown, chain stops."]]
    GateOptIn -- Yes --> MonitorB{"FINDINGS contains<br/>recommended=monitor?"}
...
**Both `SawSignalLine` and the model's raw (pre-strip) response text are now persisted**, not just used in-request and discarded. Previously, `AgentTranscript` (persisted via `recordPlaybookRunComplete`) was always built from the *cleaned* text — `respBody["text"]` gets overwritten with `esc.CleanText` before the transcript is captured — so there was no way to verify, after the fact, whether a given run's `protocol_violation` (or lack of one) was actually correct: the one field the whole mechanism depends on was never recorded anywhere. `PlaybookRun.SawSignalLine` (new column, `saw_signal_line`) and the raw transcript close that gap — `GET /api/v1/fleet/playbook-runs/{run_id}` and the incident narrative's `TriageChapter`/`RemediationChapter`/`EscalationHop` all now carry `saw_signal_line` and `AgentTranscript` is the model's actual output, signal line included.
...
**4. The fallback gate — opt-in and the only one of the four that actually gates.** Pass both `"gate_escalation": true` and `"remediation_series_id": "<series_id>"` on the *original* `/run` request and a hop that **genuinely omitted** the signal line (`SawSignalLine=false`) manufactures a `pending_gate` using that series as the forced target — instead of just warning, it pauses for operator approval, since the caller has now supplied a target to gate *into* (which the other three mechanisms never have):
...
Skipped in two independent cases: when `SawSignalLine=true` (the hop explicitly emitted `TRANSITION_TO: none`/`ESCALATE_TO: none` — a compliant, complete conclusion, not an omission — regardless of what `FINDINGS:` says) and when `FINDINGS:` recommends `monitor`/`no_changes_needed` (same escape hatch as the normal gate, nothing needs operator action either way). Before the `SawSignalLine` check existed here, an explicit `"none"` and a silently omitted line were indistinguishable to this block, so a hop that correctly concluded "nothing to hand off" still got a `pending_gate` manufactured against the caller's `remediation_series_id` — a target the model never proposed — misrepresenting its own conclusion as an unresolved handoff and logging a factually wrong "agent omitted TRANSITION_TO/ESCALATE_TO" warning. **Deduped against check 2 above**: when both fire on the same hop (as in the example — a `PlaybookType == "triage"` hop with `gate_escalation`+`remediation_series_id` both set), `warnings` gets exactly one protocol-violation entry, not two — the fallback gate only appends its own (differently-worded) message when check 2 didn't already cover the same fact, i.e. when `PlaybookType` isn't `"triage"`.

**Where this shows up later, beyond the live response**: `vault journeys` marks a `P` flag (alongside the existing `!`/`D`) and prints a boxed "PROTOCOL VIOLATION WARNING" in `vault journey <trace_id>` detail view; `vault incidents`/`GET /api/v1/incidents/{runID}` surfaces an inline `⚠ protocol violation` line under whichever chapter's hop violated the protocol, no separate Journey lookup required; `vault accuracy <fault-id>`'s `Signal types:` line breaks down exactly which of the unconditional signals (`objective_evidence:<signal>` — now signal-keyed, e.g. `objective_evidence:pod_restarted=3`, falling back to the flat `objective_evidence` bucket for older responses without `objective_evidence_signals` — `protocol_violation`, `target_drift`, `mismatch`, `unverified_evidence` (v0.28.0, content-provenance, primary-hypothesis only — see [AIGOVERNANCE.md §1.1's Layer 3](AIGOVERNANCE.md#layer-3--content-provenance-verification); its sibling `unverified_evidence_secondary` is tracked in the same distribution but does not contribute to `warning_count`) and, for faulttest catalog runs specifically, `evidence_coverage_gap`/`evidence_unconfirmed` (v0.28.0, see [ATTRIBUTION_CERTS.md §9](ATTRIBUTION_CERTS.md#9-the-clean-axis)); not `low_confidence` — see above) contributed to a cert's `warning_count`. `vault journey <trace_id>` also now fetches the raw `delegation_verification` event(s) directly and shows tool/value detail inline under each warning section — narrated-but-unconfirmed tool names under `!`, offending tool/connection-string pairs under `D` — instead of only the generic boilerplate text.
...
| `gate_reason: "trust_not_earned"` | `pending_gate` response | The playbook's series_id has no fault-stability cert on record for the gateway's configured diagnosis model (`HELPDESK_MODEL_NAME`) with a `STABLE` verdict, a `CLEAN` verdict, *and* attribution consistency — see [`ATTRIBUTION_CERTS.md`](ATTRIBUTION_CERTS.md) for what earns each. Fails closed: never having been `faulttest run --repeat N`-certified for this model counts as unearned, not as a pass. | Verified — a real query against the cert store, not a per-run signal at all | Independently evaluated; combines with the other two via `+`. Bypassed by `skip_trust_gate: true` on the request — set automatically by faulttest's own calibration runs, since they're the ones trying to *establish* the cert this check depends on; never set this for a real incident. |
...
| `evidence_warnings` + `objective_evidence_signals` | Normal (non-`pending_gate`) response | A hop has real objective evidence but emits neither `TRANSITION_TO` nor `ESCALATE_TO` | Verified | **Structurally mutually exclusive with `pending_gate`/`gate_reason`** — a hop with no escalation signal always terminates the chain right there, so a later hop's gate can never fire in the same response. Not just "by convention" — the chain loop's own exit condition makes this impossible, not merely undesigned-for. `objective_evidence_signals` (the deduplicated signal names alone, e.g. `["oom_killed"]`) is populated here too, same as on the `gate_reason` path — see [Objective-evidence gate](#objective-evidence-gate) above. |
...
| `target_drift` + `target_drift_detail` | Any response with a `connection_string` on the request | A real tool call in the run used a different `connection_string` than the run was invoked with | Verified — read directly from `tool_execution` audit events, independent of the model's text | Unconditional, runs before the crystal-ball branch. `target_drift_detail` is additive (attributes each divergent value to the tool call that produced it) — see [`MUTATION_TOOLS.md` §5.6](MUTATION_TOOLS.md#56-target-scope-drift-detection-checktargetscope). **Now feeds the CLEAN cert** (`hasCleanWarning`/`warningTypesFor` in faulttest), same tier as `protocol_violation`/`evidence_warnings` — previously it was visible on the live response but silently excluded from calibration. |
| `policy_denials` | Any response | One or more `policy_decision` events with `effect="deny"` were recorded for the hop | Verified — read directly from `policy_decision` audit events | Unconditional, accumulated across every hop (primary + auto-chained). Not itself gated or CLEAN-tracked — purely structured visibility for a denial that would otherwise only be inferable from the agent's own prose. See [`MUTATION_TOOLS.md` §5.8](MUTATION_TOOLS.md#58-structured-policy-denial-visibility-checkpolicydenials). |
| `mismatch` + `narrated_not_confirmed` | Any response | A hop's `delegation_verification` event has `Mismatch=true` — the model narrated calling a tool (or claimed a write/destructive action) that produced no matching `tool_execution` event | Verified — read directly from `delegation_verification` audit events | Unconditional, accumulated across every hop. The orthogonal counterpart to `target_drift`: drift needs a *real* tool call at the *wrong* target; this needs a *claimed* tool call that never executed at all — a hop can trip either, both or neither. Previously visible only via an undetailed `X-Audit-Mismatch` response header that the playbook-run path never read. See [`MUTATION_TOOLS.md` §5.9](MUTATION_TOOLS.md#59-fabrication-risk-visibility-checkfabricationrisk). |
| `unverified_evidence` (v0.28.0) | Any response | A hop's `delegation_verification` event has a non-empty `UnverifiedEvidence` — an `EVIDENCE` quote on the report's **primary/root-cause hypothesis** didn't match, after normalization, any real `tool_execution` output for that hop | Verified — read directly from `tool_execution` audit events | Unconditional, accumulated across every hop. The content-level sibling of `mismatch`: that checks a claimed *action* really happened, this checks a claimed *fact* really came from somewhere real. Each entry is prefixed with its owning hypothesis's own text (`"<hypothesis text> — <quote>"`). **Warn-only as of 2026-09-08** — computed and surfaced everywhere, but no longer feeds the CLEAN cert (see [ATTRIBUTION_CERTS.md §9](ATTRIBUTION_CERTS.md#9-the-clean-axis) for why). See [AIGOVERNANCE.md §1.1's Layer 3](AIGOVERNANCE.md#layer-3--content-provenance-verification). |
| `unverified_evidence_secondary` (v0.28.0) | Any response | Same check as `unverified_evidence`, for a hypothesis the model itself **rejected** (non-primary) | Verified | Unconditional, accumulated across every hop, same labeling as `unverified_evidence`. Real fabrication, still worth knowing about, but **does not feed the CLEAN cert** — found live, a rejected hypothesis's invented detail was permanently withholding CLEAN from an otherwise correctly-attributed, STABLE diagnosis (the model's actual, acted-on conclusion was not built on the fabricated quote). See [AIGOVERNANCE.md §1.1's Layer 3](AIGOVERNANCE.md#layer-3--content-provenance-verification). |
...
The triage run is recorded with `outcome: gate_pending`. The run ID is stable and can be used to retrieve the findings later via `GET /api/v1/fleet/playbook-runs/{run_id}` API call — this now also includes `gate_reason` (persisted on `PlaybookRun`, not just the ephemeral live response). Before this, `gate_reason` existed only in the single HTTP response body of the `/run` call itself; if that response wasn't captured (e.g. piped straight into a `jq` filter that discarded it), the reason a gate fired was permanently unrecoverable — not in the DB, not in the Decision Hub payload, nowhere. It also survives `proceed-escalation`: resolving a gate (approved or denied) preserves the original `gate_reason` on the record rather than clearing it, since the field is most useful to have queryable *after* the gate has been acted on, not just while it's pending.

### Troubleshooting warning signals

A quick reference for what to check first when a specific signal fires — grounded in real root causes found during manual verification of this system, not hypothetical failure modes. Co-located with the reference table above rather than a separate document, so it can't silently drift out of sync as signals are added or change shape.

| Signal | Likely root cause(s) | First thing to check |
|---|---|---|
| `objective_evidence:<signal>` (gated or `evidence_warnings`) | The playbook's guidance doesn't tell the model what to do with this specific tool evidence or the model saw the evidence and didn't connect it to the diagnosis. | `vault journey <trace_id>` for the raw evidence event's `signal`/`detail`; compare against the playbook's `FINDINGS:` template — does it have a structured field for this evidence category at all? |
| `protocol_violation` | The model's response was truncated (hit a token limit) before reaching the required trailing `TRANSITION_TO:`/`ESCALATE_TO:` lines or it got derailed by an unexpected finding (e.g. "ticket said X, but I found Y") and never returned to the required output format. | Fetch the persisted `agent_transcript` via `GET /api/v1/fleet/playbook-runs/{run_id}` — the raw, un-stripped text. Check whether it's genuinely cut off mid-sentence (token limit — a config/infra fix) or complete-but-noncompliant (a prompt/guidance gap). |
| `target_drift` (+ `target_drift_detail`) | The `connection_string`/`namespace` in the request is an alias the resolution layer doesn't recognize (e.g. a Docker container name rather than the infra config's canonical key) — the agent or the model from prompt context, fell back to a different real, known server instead. | `vault journey <trace_id>` shows `tool → connection_string` pairs directly under TARGET DRIFT WARNING. Confirm the request's target actually resolves against infra config — see [`MUTATION_TOOLS.md` §5.6](MUTATION_TOOLS.md#56-target-scope-drift-detection-checktargetscope) for exactly which forms resolve (key, `name`, `container_name`). |
| `mismatch` (+ `narrated_not_confirmed`) | Usually one of two things: (1) the target didn't resolve at all, so the tool call hard-rejected before ever executing and the model fabricated a plausible-sounding result instead of reporting the failure; or (2) genuine model unreliability. **A useful elimination**: if `mismatch` fired, a `policy_decision` denial has *already been ruled out* as the explanation — the narration check is suppressed whenever a real denial exists for the hop (see [§5.7](MUTATION_TOOLS.md#57-narrated-but-unconfirmed-tool-calls-read-action-coverage)), so seeing `mismatch=true` means something else is going on, not a policy config issue. | `vault journey <trace_id>` shows narrated tool names under FABRICATION WARNING. Check whether the connection target resolves at all (same check as `target_drift`, above) before assuming pure model unreliability. |
| `policy_denials` | Not itself a CLEAN failure — structured context explaining why a tool call didn't happen, most commonly a missing `X-Purpose` header or a role/resource-tag mismatch in the policy file. | Read `message` directly on the response — it self-describes what was blocked. Cross-check the request's `X-Purpose` header and the resource's tags against the policy rules. |
| `unverified_evidence` / `unverified_evidence_secondary` | The model quoted something that isn't literally in any tool's recorded output — either genuinely invented, a paraphrase instead of a verbatim quote or (fixed 2026-09-07) a compound quote joining two real facts with "and"/"," that used to get flagged as one indivisible blob even when every part was real. Truncated tool output (long logs) can also hide a real quote past the recorded window. | The flagged quote is printed inline (`vault incidents`/live response), already labeled with its owning hypothesis's text — no extra lookup needed. If `unverified_evidence_secondary` (non-primary hypothesis) fired, the primary conclusion is unaffected and CLEAN isn't blocked; if `unverified_evidence` (primary) fired, check whether the quote is a paraphrase (tighten the playbook's `EVIDENCE:` instruction) or genuinely fabricated (a model-reliability finding). |
| `gate_reason: "low_confidence"` / `confidence_warning` | Self-reported by the model — genuinely ambiguous evidence, not a code-level bug. Deliberately excluded from CLEAN; tracked instead by the Evaluation stability axis (judge/confidence variance across repeated runs). | `vault accuracy <fault-id>` for judge/confidence variance history. Consistently low confidence on the *same* fault across repeats points to a prompt/guidance gap, not one-off ambiguity. |
| `gate_reason: "trust_not_earned"` | No cert with a `STABLE` verdict, a `CLEAN` verdict, *and* attribution consistency exists yet for this playbook series under the currently-configured diagnosis model — a bootstrapping state, not a failure. | Run `faulttest run <fault-id> --repeat N` to establish a cert or check `vault list`/`vault accuracy` for current cert status by model. |
...
### What "cross-domain" is actually buying you

A **hop** is one playbook run in a chain. A **domain** is the boundary between agents that each own a different slice of the stack — database internals, host/container state, K8s Pod state — with different tools, different failure vocabularies and [no shared visibility](PRINCIPLES.md#6-context-isolation-through-sub-agents) into each other's world. `TRANSITION_TO` moves to the next hop *within* a domain (triage handing off to its own remediation counterpart, same agent family). `ESCALATE_TO` crosses one: the current agent has hit the edge of what it can see and is handing the investigation to an agent that can see the next layer down.

This is the specific cost the escalation graph exists to remove. In a manually-run [incident](INCIDENTS.md), crossing that boundary means a person: someone notices that the Database Agent can't explain a symptom on its own, decides who owns the next layer, writes up what's known so far and waits for that person to pick it up. Sometimes from a different team. Sometimes in a war room. Either way, that's where you start paying the [hand-off tax](https://levelup.gitconnected.com/the-hand-off-tax-part-1-what-a-3-hop-escalation-chain-actually-buys-you-d708f4912d4c): the routing, the re-explaining, more people involved, details likely lost in translation. This tax is paid on every hop, whether or not the underlying fix is hard. 

aiHelpDesk escalation graph makes that hand-off process structural instead of situational: the next agent is invoked automatically, with the full prior hop's findings threaded in via `prior_run_id` (see [Continuity threading](#continuity-threading) below), so no mis-interpretation, no details lost in translation. And yes, the whole chain is captured as one incident, not a scattered thread of separately-initiated tickets.
...
Full transcript, including a side-by-side run with the escalation graph deliberately disabled (`Crystal Ball` mode) so the same fault gets diagnosed by the same model with no chain to lean on: [`docs/samples/SAMPLE015.md`](samples/SAMPLE015.md), covered in ["The Hand-Off Tax, Part 1: What a 3-Hop Escalation Chain Actually Buys You"](https://levelup.gitconnected.com/the-hand-off-tax-part-1-what-a-3-hop-escalation-chain-actually-buys-you-d708f4912d4c) and ["Part 2: We Turned Off the Playbooks and Watched"](https://itnext.io/the-hand-off-tax-part-2-we-turned-off-the-playbooks-and-watched-fade1e510391).

**Two hops, one handoff that has to change targets mid-chain** (`db-replica-disconnected`):  
The Database Agent finds an inactive replication slot retaining WAL, with zero rows in `pg_stat_replication`. The replica is gone, not lagging (which is a different failure scenario). It has no tools that can reach the replica's own container, so it escalates to `pbs_sysadmin_replica_connectivity_triage`.  The trick here is that upon the handoff, the new playbook can't just use the primary's own connection string, because the thing that needs inspecting now is a *different* server. The Database Agent's `TARGET:` signal line carries the replica's own `host:port` forward and the SysAdmin agent picks up from there: `check_host`/`get_host_logs` against the replica's own container, finding it healthy and running. 

In [this particular case](samples/SAMPLE019.md) the real problem turned out to be the *primary* rejecting the replica's reconnection attempts at the `pg_hba.conf` layer, invisible from the Database Agent's own vantage point. The catch here is not to restart anything (which is a solution to some of the replication problems), but not here because nothing about the replica itself is broken and so the chain correctly stops at diagnosis (`ESCALATE_TO: none`) rather than transitioning into a restart nothing needs.

Both chains are fully attributed after the fact: `vault incidents <run_id>` prints every hop's own findings and hypotheses under its own `── TRIAGE`/`── ESCALATION` heading and — since v0.26.0 — every hop in the chain earns its own independent [fault-stability cert](CONSISTENCY.md#certification-scope-every-hop-in-a-chain-not-just-the-entry-point-v0260), not just the fault's declared entry point. A chain crossing three domains is exactly as auditable, hop by hop, as a single-domain run.
...
```json
{
  ...
  "incident_record": {
    "incident_id":       "inc_9f2a7b1c",
    "entry_run_id":      "plr_a3f7c1b2",
    "origin":            "real",
    "status":            "resolved",
...
`incident_record` is the v0.29 incidents-table row for this run — present only when one exists (any run predating that redesign or never tracked as an incident, simply omits this field entirely). Note that `incident_record.incident_id` (`inc_9f2a7b1c`) and the top-level `incident_id` (`plr_a3f7c1b2`) are deliberately different values: the top-level one is this run's own ID, unchanged since before v0.29; the nested one identifies a separate row in the incidents table. `bundle_path`/`draft_playbook_id` are empty here because this story never actually called `create_incident_bundle` — see [Step 7](#step-7--vault-the-knowledge-compounds) for what populates them.

### Step 7 — Vault: the knowledge compounds

This incident — its chain of thought, the operator's gate reason, the remediation steps and the confirmed diagnosis — is now part of the Vault's institutional memory, in two independent ways.

**The accuracy signal**, from the feedback submitted in Step 5:
...
The next operator who encounters a lock chain gets the same agent, the same Playbook guidance and now a 95.7% confirmed accuracy rate behind the hypothesis it produces. The drift command (`faulttest vault drift`) will flag it if that rate starts to fall — for example, if a PostgreSQL upgrade changes lock behaviour and the existing hypotheses become less reliable. That is the signal to update the Playbook guidance — but the update itself doesn't happen through the accuracy number alone.

**The content signal**, which is how the *update* actually happens: when this incident's entry playbook run concludes (`resolved`/`escalated`/`transitioned`), the gateway can synthesise a Playbook *draft* directly from the run's own audit trail — every tool call, in the order the agent actually made them — and save it to the Vault as an inactive candidate, pinned to `pbs_lock_chain_triage` so it improves the existing series rather than starting a new one. A human reviews it (`vault diff`, `vault drafts`) and only becomes live once explicitly activated (`vault activate`). This is the mechanism that actually rewrites Playbook guidance in response to what happened here, not just a number telling you it's time to. See [INCIDENTS.md § From Incident to Vault: the Full Path](INCIDENTS.md#from-incident-to-vault-the-full-path) for the complete diagram — bundle creation, `origin` tagging and the incidents-table row this specific incident's `incident_record` (Step 6, above) came from — and [VAULT.md § Draft Review Flow](VAULT.md#draft-review-flow) for the review/activation mechanics themselves.
