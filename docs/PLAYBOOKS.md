...
| `unverified_evidence` (v0.28.0) | Any response | A hop's `delegation_verification` event has a non-empty `UnverifiedEvidence` — an `EVIDENCE` quote on the report's **primary/root-cause hypothesis** didn't match, after normalization, any real `tool_execution` output for that hop | Verified — read directly from `tool_execution` audit events | Unconditional, accumulated across every hop. The content-level sibling of `mismatch`: that checks a claimed *action* really happened, this checks a claimed *fact* really came from somewhere real. Each entry is prefixed with its owning hypothesis's own text (`"<hypothesis text> — <quote>"`). **Feeds the CLEAN cert.** See [AIGOVERNANCE.md §1.1's Layer 3](AIGOVERNANCE.md#layer-3--content-provenance-verification). |
