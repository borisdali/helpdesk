...
| `has_unverified_evidence` | `true` when at least one `delegation_verification` event in this Journey has a non-empty `unverified_evidence` — a hypothesis `EVIDENCE` quote didn't match, after normalization, any real `tool_execution` output recorded for that hop (v0.28.0, content-provenance). Independent of `has_mismatch`/`has_target_drift`/`has_protocol_violation`: `has_mismatch` checks a claimed *action* really happened, this checks a claimed *fact* really came from somewhere real. Omitted (falsy) when clean. See [AIGOVERNANCE.md §1.1's Layer 3](AIGOVERNANCE.md#layer-3--content-provenance-verification). |
...
| `unverified_evidence` | 9 | A hypothesis `EVIDENCE` quote didn't match any real `tool_execution` output for that hop (v0.28.0, content-provenance). Tied with the other three at priority 9, not ranked. Use `has_unverified_evidence` on the Journey object, not just the `outcome` string, to detect it alongside the others on the same trace. Warn-only, no gate forced — see [AIGOVERNANCE.md §1.1's Layer 3](AIGOVERNANCE.md#layer-3--content-provenance-verification). |
