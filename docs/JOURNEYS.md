...
| `has_unverified_evidence` | `true` when at least one `delegation_verification` event in this Journey has a non-empty `unverified_evidence` — a hypothesis `EVIDENCE` quote on the **primary/root-cause hypothesis** didn't match, after normalization, any real `tool_execution` output recorded for that hop (v0.28.0, content-provenance). Independent of `has_mismatch`/`has_target_drift`/`has_protocol_violation`: `has_mismatch` checks a claimed *action* really happened, this checks a claimed *fact* really came from somewhere real. Omitted (falsy) when clean. See [AIGOVERNANCE.md §1.1's Layer 3](AIGOVERNANCE.md#layer-3--content-provenance-verification). Fabrication on a *non-primary* (rejected) hypothesis is tracked separately as `unverified_evidence_secondary` — surfaced on incident-narrative chapters (see [API.md](API.md#get-apiv1incidentsrunid)/[INCIDENTS.md](INCIDENTS.md)), not as a Journey-level flag or outcome, since it doesn't indict the model's actual, acted-on conclusion the way this field does. |
...
| `unverified_evidence` | 9 | A hypothesis `EVIDENCE` quote on the primary/root-cause hypothesis didn't match any real `tool_execution` output for that hop (v0.28.0, content-provenance). Tied with the other three at priority 9, not ranked. Use `has_unverified_evidence` on the Journey object, not just the `outcome` string, to detect it alongside the others on the same trace. Warn-only, no gate forced — see [AIGOVERNANCE.md §1.1's Layer 3](AIGOVERNANCE.md#layer-3--content-provenance-verification). Fabrication on a non-primary hypothesis does not elevate this outcome — see `unverified_evidence_secondary` in [API.md](API.md#get-apiv1incidentsrunid). |
