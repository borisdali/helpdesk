...
Optional:
  ...
  --replan                     On schema drift, replan using the job's stored plan_description and re-execute
...
Use `--replan` to automatically call the planner when drift is detected, instead of aborting:

```bash
./fleet-runner --job-file jobs/weekly-sweep.json --replan
```

If `tool_snapshots` do not match the live registry, fleet-runner uses the stored `plan_description` and `plan_target_hints` to generate a fresh plan, then re-checks drift on the new plan. The job continues with the updated definition. Without `--replan`, drift causes a hard abort.

**Requirement:** the job file must have been generated by the planner (so `plan_description` is populated). Hand-crafted files without `plan_description` cannot be auto-replanned — run `--refresh-snapshots` instead.
