...
| `gate_reason` | When gate was forced by the gateway | `"low_confidence"` — primary hypothesis confidence was below 50%; `"objective_evidence:<signal>"` (e.g. `"objective_evidence:pod_restarted"`) — a tool call in the hop produced deterministic, code-derived evidence (a real K8s pod restart/OOM kill, not something the model reported about itself); or both, joined with `+` (e.g. `"low_confidence+objective_evidence:pod_restarted"`) when a hop trips both checks — they're evaluated independently, so one firing never suppresses the other. Either way, the gate fired automatically even without `gate_escalation=true`. |
