...
| `gate_reason` | When gate was forced by the gateway | Three independent causes, `+`-joined when more than one fires on the same hop (e.g. `"low_confidence+objective_evidence:pod_restarted"`): `"low_confidence"` (primary hypothesis confidence below 50%), `"objective_evidence:<signal>"` (a tool call produced deterministic, code-derived evidence — e.g. a real K8s pod restart/OOM kill — not something the model reported about itself), and `"trust_not_earned"` (the playbook's series has no `STABLE`+`CLEAN` fault-stability cert on record for the current model — a check on the *playbook*, not the individual run; see [ATTRIBUTION_CERTS.md §9](ATTRIBUTION_CERTS.md#9-the-clean-axis)). All three are evaluated independently — none suppresses the others. Either way, the gate fired automatically even without `gate_escalation=true`. |

`remediation_preview` lets operators see exactly what remediation will do — its name, intent, and default approval mode — before clicking approve. `diagnostic_report` records the structured reasoning behind the triage conclusion for later audit review. `gate_reason` signals that the gate was not requested by the caller but enforced by the gateway — see [Objective-evidence gate](PLAYBOOKS.md#objective-evidence-gate) and the [warning signals reference table](PLAYBOOKS.md#warning-signals-reference) for the full explanation of every force-gate cause and how they relate to `evidence_warnings`.

Note: a hop can also carry real, code-derived tool evidence *without* a Decision Hub entry ever being created — when the hop emits no `TRANSITION_TO`/`ESCALATE_TO` at all, there is no next-hop candidate to gate approval for, so nothing surfaces here. That case is visible only on the raw playbook-run response's `evidence_warnings` field, not in the Decision Hub — see [Objective-evidence gate](PLAYBOOKS.md#objective-evidence-gate).
