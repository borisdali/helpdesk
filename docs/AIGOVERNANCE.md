...
| `has_mismatch`/`trace_id` on the incident narrative chapter | `GET /api/v1/incidents/{runID}` | Surfaced inline on the specific TRIAGE/ESCALATION/REMEDIATION chapter that hop belongs to — computed independently of the journey object, scoped to that hop's own execution window rather than the whole trace, so one hop's mismatch does not flag every chapter sharing that trace_id; no separate Journey lookup needed; `vault incidents` prints it as an inline `⚠ unverified` line |
