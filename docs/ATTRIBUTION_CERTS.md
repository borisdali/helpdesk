...
### What counts as a warning, and what deliberately doesn't

`CLEAN` tracks exactly seven signals, all *verified* (code-derived from real tool output or the fault catalog's own declared expectations), not self-reported:
...
6. `evidence_coverage_gap` true (faulttest catalog only, v0.28.0) — the fault declares `expected_diagnosis.objective_evidence_signal`, but that signal never fired at all: the agent's tool calls never reached the code path that would produce it. A coverage problem — tooling or prompt didn't get there — not a confirmation problem.
7. `evidence_required_but_unconfirmed` true (faulttest catalog only, v0.28.0) — same declared-signal check as #6, but the signal *did* fire and still wasn't confirmed: the agent had the real data and didn't demonstrably engage with it. Split from #6 deliberately, on real feedback — both correctly fail the fault's `passed` verdict, but they point at different fixes (tooling/prompt coverage vs. confirmation/quoting logic), and a single flat flag hid which one a given red run needed. Distinct from #1/#2 above: those track the *production* force-gate signal firing at all; these two track the *fault catalog's own* declared-signal check specifically.
...
Two more signals are excluded for a different reason — both are `unverified_evidence`-family, content-provenance checks (see [AIGOVERNANCE.md §1.1's Layer 3](AIGOVERNANCE.md#layer-3--content-provenance-verification)): an EVIDENCE quote didn't match, after normalization, any real `tool_execution` output recorded for that hop.

- `UnverifiedEvidenceSecondary` — flags a hypothesis the model itself *rejected* rather than the one it acted on. Found live: a STABLE, correctly-attributed diagnosis could never earn CLEAN because a rejected alternative hypothesis cited an invented detail (a model claimed a real log said "terminating walreceiver due to timeout" when it actually said "due to administrator command") — the same "right conclusion, gated identically to a wrong one" mistake [Layer 4's own history](OBJECTIVE_EVIDENCE.md#8-history-from-gate-on-presence-to-gate-on-contradiction) already fixed once, one layer over. Still real fabrication, still recorded and surfaced (its own `warning_distribution` bucket, its own line in the incident narrative, prefixed with the owning hypothesis's text) — just not CLEAN-blocking, since the model's actual, acted-on conclusion was not built on it. Never counted toward `CLEAN` since it shipped.
- `UnverifiedEvidence` (primary/root-cause hypothesis) — counted toward `CLEAN` from ship (v0.28.0) until **2026-09-08**, when it moved to warn-only, matching its secondary sibling. Reason: multiple consecutive live-testing rounds on the same three replication faults each surfaced a *new*, genuine citation-formatting variant a model can produce while citing real, correct content — compound quotes joined by unusual connector phrases, backslash-escaped inner quotes, narration interposed between/after quotes naming a source view, separator-punctuation swaps (`=`/`:` for psql's `|`), and dropped embedded quote marks — plus a distinct, structural false positive unrelated to citation style: `toolResultMaxLen` head-truncation of a chatty, repetitive log discarding exactly the tail content a hop's own hypothesis correctly quoted (see [MUTATION_TOOLS.md §5.11](MUTATION_TOOLS.md#511-content-provenance-verification-checkevidenceprovenance) for the full history and each fix). Every case found was a genuine false positive, never a missed real fabrication — but the pattern of new variants still appearing after multiple hardening rounds means the false-positive surface isn't provably closed, and gating a v0.27-era CLEAN cert on an unproven new signal reads as a regression to customers who never touched this feature. Still computed, still surfaced everywhere (CLI, `vault`, `warning_distribution`) — only removed from the `WarningCount`/`isClean()` boolean. Revisit once proven stable across a broader corpus of fault families and runs.
...
### What to do when you see `unverified_evidence` on an otherwise-`Clean: yes` cert?
...
### Gating real incidents
This is the axis's second, larger purpose beyond reporting: `cmd/gateway/playbooks.go`'s `trustNotYetEarnedForceGate` forces a `pending_gate` (`gate_reason: "trust_not_earned"`) on any real playbook run whose series has no cert on record — for the gateway's configured diagnosis model — that is `STABLE`, `CLEAN`, **and attribution-consistent** (§3's `attribution_consistent` field). Fails closed: never having been `faulttest run --repeat N`-certified counts as "not earned," not as a pass, and **all** known certs for a series must clear all three bars (a series can map to multiple faults; one bad one is enough to withhold trust). Attribution consistency joined this gate in v0.25.0 after a live-confirmed gap: `db-wal-disk-full-k8s` once scored `STABLE(5)` at 100% pass rate while its attribution split 3/5 `wal-disk-full` vs. 1 `clean-shutdown` vs. 1 `oom-kill` — if also `CLEAN`, that cert would have passed the old two-condition gate and auto-chained unattended despite the model disagreeing with itself on root cause a third of the time. Attribution consistency doesn't get its own badge or new cert section the way CLEAN did (see §9 above on why CLEAN earned separate treatment) — it's a diagnostic-quality judgment already computed from the same `EvalResult` batch as Outcome/Evaluation (§1, `attr=X(consistent/split)`); it just needed the same gate consumer to read it. See [`PLAYBOOKS.md`'s warning signals reference](PLAYBOOKS.md#warning-signals-reference) for the full response-field contract, including how `skip_trust_gate` lets faulttest's own calibration runs bypass a check they're specifically trying to satisfy.

The same three-condition bar is exposed as `FaultStabilityCert.EarnsTrust()` (v0.25.0) so this gate and the cert store's own regression detection — did a recertification just cause a fault+model to *stop* clearing this exact bar — read one shared fact rather than two independently-maintained copies of the same three-field check. See [CONSISTENCY.md's Versioning & history fields](CONSISTENCY.md#versioning--history-fields-v0250) for the regression-alert mechanism this enables, and cert history for tracking whether a cert keeps clearing this bar over time, not just right now.
