...
**`warning_distribution`** — a sixth cert column, mirroring `attribution_distribution`'s exact shape and purpose (§3): a per-type run count, e.g. `{"objective_evidence": 1, "protocol_violation": 2}`. `WarningCount` alone can't tell an operator *which* of the three signals above fired; this can. Displayed via `vault accuracy <fault-id>`'s `Warning types:` line, directly under `Clean:`. Not shown in `vault list` — that view is already dense (label + runs + clean + attribution + age share one column), so the breakdown lives only in the detail view.
