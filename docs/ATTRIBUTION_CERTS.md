...
**What counts as a warning, and what deliberately doesn't.** `CLEAN` tracks exactly four signals, all *verified* (code-derived from real tool output), not self-reported:
...
**`warning_distribution`** — a sixth cert column, mirroring `attribution_distribution`'s exact shape and purpose (§3): a per-type run count, e.g. `{"objective_evidence:pod_restarted": 1, "protocol_violation": 2, "target_drift": 1}`. `WarningCount` alone can't tell an operator *which* of the four signals above fired; this can. The `objective_evidence` bucket is signal-keyed (`objective_evidence:<signal>`, e.g. `objective_evidence:oom_killed`) when the response carries the structured `objective_evidence_signals` field (see [`PLAYBOOKS.md`'s Objective-evidence gate](PLAYBOOKS.md#objective-evidence-gate)); it falls back to the flat `objective_evidence` bucket for responses recorded before that field existed, so older data doesn't silently vanish from the distribution. Displayed via `vault accuracy <fault-id>`'s `Warning types:` line, directly under `Clean:`. Not shown in `vault list` — that view is already dense (label + runs + clean + attribution + age share one column), so the breakdown lives only in the detail view.
