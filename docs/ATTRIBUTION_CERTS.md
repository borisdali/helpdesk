...
**What counts as a warning, and what deliberately doesn't.** `CLEAN` tracks exactly eight signals, all *verified* (code-derived from real tool output or the fault catalog's own declared expectations), not self-reported:
...
8. `unverified_evidence` true (v0.28.0) — a hop's `delegation_verification` event had a non-empty `UnverifiedEvidence`: an EVIDENCE quote on the **primary/root-cause hypothesis** didn't match, after normalization, any real `tool_execution` output recorded for that hop (see [AIGOVERNANCE.md §1.1's Layer 3](AIGOVERNANCE.md#layer-3--content-provenance-verification), content-provenance verification). Unlike #6/#7, this applies to *any* run, not just faulttest catalog faults with a declared signal — the content-level sibling of #5 (`mismatch`): that checks a claimed action really happened, this checks a claimed fact really came from somewhere real. Tied at the same Journey-outcome priority (9, `unverified_evidence`) as the other four "don't trust this output as-is" signals. Scoped to the primary hypothesis specifically as of the same v0.28.0 pass that added it — see the exclusion note immediately below for `UnverifiedEvidenceSecondary`, its non-primary sibling.
...
A third signal is excluded for a different reason: `UnverifiedEvidenceSecondary` — the same content-provenance check as #8, but for a hypothesis the model itself *rejected* rather than the one it acted on. Found live: a STABLE, correctly-attributed diagnosis could never earn CLEAN because a rejected alternative hypothesis cited an invented detail (a model claimed a real log said "terminating walreceiver due to timeout" when it actually said "due to administrator command") — the same "right conclusion, gated identically to a wrong one" mistake [Layer 4's own history](OBJECTIVE_EVIDENCE.md#8-history-from-gate-on-presence-to-gate-on-contradiction) already fixed once, one layer over. Still real fabrication, still recorded and surfaced (its own `warning_distribution` bucket, its own line in the incident narrative, prefixed with the owning hypothesis's text the same way #8's is) — just not CLEAN-blocking on its own, since the model's actual, acted-on conclusion was not built on it.
