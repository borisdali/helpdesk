...
| `unverified_evidence` | Hypothesis `EVIDENCE` quotes (v0.28.0) that didn't match, after normalization, any real `tool_execution` output recorded for this hop. Independent of `mismatch` — the tool call itself can be real and correctly confirmed while the quote is fabricated. Content-provenance, fabrication-detection Layer 3. See [MUTATION_TOOLS.md §5.11](MUTATION_TOOLS.md#511-content-provenance-verification-checkevidenceprovenance) |
