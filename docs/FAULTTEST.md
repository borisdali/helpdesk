...
| `db-idle-in-tx-blocker` | SQL: opens a transaction, performs a write, holds it open; three victim sessions queue on the same row lock |
...
| `db-idle-in-tx-blocker` | Idle-in-transaction root blocker with lock queue | high |
...
| `db-idle-in-tx-blocker` | `pbs_idle_blocker_triage` | db |
...
| `db-idle-in-tx-blocker` | `SELECT count(*) = 0 FROM pg_stat_activity WHERE state = 'idle in transaction' AND query LIKE '%_faulttest_lock_chain%' AND pid <> pg_backend_pid()` |
